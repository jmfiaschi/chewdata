...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    return Err("the toml feature is required for this example. Please enable it in your Cargo.toml file. cargo example EXAMPLE_NAME --features toml".into());
}
...
async fn run() -> io::Result<()> {
    ...
    assert_eq!(
        expected,
        result.clone().search("/*/line/number")?.unwrap_or_default(),
